...
public class Node
{
    ...
    {
        creator = _creator;
        tempLinkedNodeList = linkedNodes.ConvertAll(x => _creator.FindWithPos(x)).FindAll(x => x != null);
    }

    public void Init(Vector2 _position, PathNodes _creator)
    {
        creator = _creator;
        tempLinkedNodeList = new List<Node>(); ;
